    ...
    <form action="{{ route('admin.kelas.update', $kelas) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="mata_kuliah_id" required
                ...
                <option value="{{ $mk->id }}" {{ $kelas->mata_kuliah_id == $mk->id ? 'selected' : '' }}>{{ $mk->nama_mata_kuliah }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
            <label class="block text-gray-600 text-xs font-semibold uppercase tracking-wider mb-2">Mentor</label>
            <select name="mentor_id"
                class="w-full px-3 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm">
                <option value="">Pilih Mentor (Opsional)</option>
                @foreach ($mentors as $m)
                <option value="{{ $m->id }}" {{ $kelas->mentor_id == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
                @endforeach
            </select>
        </div>
