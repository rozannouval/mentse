...
<head>
    ...
    <title>Mentor - @yield('title', 'Dashboard')</title>
    ...
    <div class="w-64 bg-slate-800 min-h-screen text-white flex flex-col justify-between">
        <div>
            ...
            <nav class="mt-6 px-4 space-y-2">
                <a href="{{ route('mentor.dashboard') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('mentor.dashboard') ? 'bg-amber-600 font-semibold' : '' }}">
                    Dashboard
                </a>
                <a href="{{ route('mentor.sesi.index') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('mentor.sesi.*') ? 'bg-amber-600 font-semibold' : '' }}">
                    Sesi Mentoring
                </a>
            </nav>
        </div>
        <div class="p-4 border-t border-slate-700">
            <div class="text-sm font-medium mb-2 text-slate-400">Login sebagai: Mentor</div>
