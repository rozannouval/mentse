    ...
    <div class="w-64 bg-slate-800 min-h-screen text-white flex flex-col justify-between">
        <div>
            ...
            <nav class="mt-6 px-4 space-y-2">
                <a href="{{ route('admin.dashboard') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 font-semibold' : '' }}">
                    Dashboard
                </a>
                <a href="{{ route('admin.kelas.index') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('admin.kelas.*') ? 'bg-blue-600 font-semibold' : '' }}">
                    Manajemen Kelas
                </a>
                <a href="{{ route('admin.mata-kuliah.index') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('admin.mata-kuliah.*') ? 'bg-blue-600 font-semibold' : '' }}">
                    Mata Kuliah
                </a>
                <a href="{{ route('admin.users.index') }}" class="block py-2.5 px-4 rounded transition duration-200 hover:bg-slate-700 {{ request()->routeIs('admin.users.*') ? 'bg-blue-600 font-semibold' : '' }}">
                    Data User
                </a>
            </nav>
        </div>
